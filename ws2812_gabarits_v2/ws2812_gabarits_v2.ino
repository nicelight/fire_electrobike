...
#define NUM_LEDS    60
// скорость работы пламени ( оптимально 20 - 300 )
#define FRAMES_PER_SECOND 100
// как быстро нарастает желтый на поворотнике ( 50 - очень быстро, 1000 - очень медленно)
#define TURNS_SPEED 200
// как долго желтый светится на поворотнике, прежде чем потухнуть
// это время не должно быть меньше чем TURNS_SPEED
#define TURNS_DELAY 1000
// как долго желтый НЕ светится на поворотниках
#define TURNS_BLACK 500
  ...
  switch (state) {
    ...
    case 2:
      switch (left_auto) {
        case 0:
          all_black();
          leftMs = ms + TURNS_BLACK;
          left_auto = 5;// GO
          break;
        // отрисовка бегущей желтой точки
        case 5:
          if ((ms - leftMs) > TURNS_BLACK) { // спустя пол секунды
            leftMs = ms;
            // зажигаем все
    ...
    case 3:
      switch (right_auto) {
        case 0:
          all_black();
          rightMs = ms + TURNS_BLACK;
          right_auto = 5;
          break;
        // отрисовка бегущей желтой точки
        case 5:
          if ((ms - rightMs) > TURNS_BLACK) {
            rightMs = ms;
